// Truth Seekers - WebRTC P2P Implementation with Skyway
// This version uses Skyway (a more reliable WebRTC service) for peer-to-peer connections
...
async function initializeSkyway() {
  try {
    // Using Skyway's free API key for this demo
    // In production, you should get your own key from https://skyway.ntt.com/
    const apiKey = '4d83a0b3-1e3e-4d4e-8231-8d1a8e3310e6';
    
    // Create a new Peer with a random ID
    skyway = await Peer.create({key: apiKey});
    ...
    skyway.on('connection', (conn) => {
      console.log('New connection from: ' + conn.remoteId);
      ...
      if (isHost) {
        connections[conn.remoteId] = conn;
      } else {
...
function setupConnectionHandlers(conn) {
  ...
  conn.once('open', () => {
    console.log('Connection established with: ' + conn.remoteId);
  ...
  conn.once('close', () => {
    console.log('Connection closed with: ' + conn.remoteId);
    
    // If we're the host, remove this connection from our list
    if (isHost) {
      delete connections[conn.remoteId];
      
      // Remove the player from the game state
      const playerIndex = gameState.players.findIndex(p => p.peerId === conn.remoteId);
      if (playerIndex !== -1) {
...
function createGame() {
  ...
  document.getElementById('hostPeerId').textContent = myPeerId;
  ...
  loadQuestions().then(() => {
    ...
    // Enable start game button when at least one player has joined
...
// Generate a QR code for easy joining
function generateQRCode() {
  const qrContainer = document.getElementById('qrCodeContainer');
  qrContainer.innerHTML = '';
  
  // Create join info object
  const joinInfo = {
    roomCode: roomCode,
    hostId: myPeerId
  };
...
async function joinGame() {
  ...
  try {
    // Connect to the host
    myConnection = await skyway.connect(hostId);
...
function handleQuestionData(data) {
  document.getElementById('questionNumberDisplay').textContent = `Vraag ${data.questionIndex}`;
  document.getElementById('questionContentDisplay').textContent = data.questionContent;
